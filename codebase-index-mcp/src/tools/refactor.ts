...
export function buildRefactorTools(deps: CodebaseIndexDeps): AnyToolDefinition[] {
  ...
  const refactorReplacePreview = defineTool({
    ...
    description: "Preview bulk replacements with scope and type-ownership guards. findMode='literal' (default) matches `find` as plain text; findMode='regex' treats `find` as a regular expression and substitutes capture-group backreferences in `replaceExpression` — numbered ($1..$99), whole-match ($&), named ($<name> or ${name}), and a literal `$` via $$ — ideal for context-preserving bulk edits in one pass. A backreference to a group that did not match is flagged `unsubstituted_backreference` and blocked at apply (it is never silently written). guards.allowOwnerTypes is proven from the C# AST — the type that OWNS the referenced member (instance/static/base/`this` receivers, object initializers, and one nested hop), not the class the code sits in; for other languages it is the enclosing type by text scan. Only a proven DIFFERENT owner is dropped (see rejectedSites); a site whose owner cannot be proven is kept, flagged `ambiguous_target` and explained in ambiguousReasons, so it is visible but never applied. profile='nano' returns only match count + affected files (fastest blast-radius check); 'compact' omits before/after text; 'standard' (default) returns full hunk content.",
