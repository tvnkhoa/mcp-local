...
export function buildReadMetadataTools(deps: CodebaseIndexDeps): AnyToolDefinition[] {
  ...
  const queryDocs = defineTool({
    ...
    inputSchema: {
      ...
      properties: {
        repoId: { type: "string" },
        mode: { type: "string", enum: ["search", "stale", "coverage", "drift", "links", "behind", "language"] },
        query: { type: "string" },
        ...
        contentTypes: { type: "array", items: { type: "string", enum: ["heading", "prose", "code_block"] }, minItems: 1, maxItems: 3, description: "mode=search only: which doc section kinds may answer. Defaults to all three. 'prose' is a body section chunked on heading boundaries (~512 tokens, tables kept whole); pass ['heading','prose'] to exclude fenced samples and diagrams." },
        includeArchived: { type: "boolean", description: "mode=search and mode=behind: include documents marked archived or superseded (by an archive/superseded path segment, or a **Status** line). Off by default — an archived doc is not a current state." },
        minRatioPercent: { type: "number", minimum: 0, maximum: 100, description: "mode=language only: minimum share of non-ASCII letters in a chunk before it counts as un-normalized (default 0.5)." },
        ...
        maxPerFile: { type: "integer", minimum: 0, maximum: 20, description: "mode=search only: at most this many chunks from any one file (default 1, 0 disables). A five-row budget spent on one file five times tells you less than five files." },
