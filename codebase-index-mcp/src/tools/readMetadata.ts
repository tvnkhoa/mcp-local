...
export function buildReadMetadataTools(deps: CodebaseIndexDeps): AnyToolDefinition[] {
  ...
  const queryDocs = defineTool({
    ...
    inputSchema: {
      ...
      properties: {
        repoId: { type: "string" },
        mode: { type: "string", enum: ["search", "stale", "coverage", "drift", "links", "behind"] },
        query: { type: "string" },
