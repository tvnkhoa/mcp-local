...
search_symbols(query: "<identifier>", strategy: "name", profile: "nano")
  → got symbolId
      → get_symbol_context_pack(repoId, name, profile: "compact")   ← use for quick overview (takes name, NOT symbolId)
...
```
get_folder_summary(repoId, folderPath, maxFiles: 50)   ← no profile param on this tool
...
search_symbols(query: "<symbol>", strategy: "name", profile: "nano")
  → symbolId
      → find_impact_files(repoId, filePath, view: "files", profile: "nano")   ← file count + top files
      → find_impact_files(repoId, filePath, view: "surface", profile: "compact")  ← caller surface per symbol
```

### Goal: Safe refactoring

```
find_impact_files(repoId, filePath, view: "files", profile: "nano")  ← scope check
...
```
detect_changes(repoId, policy: "strict-review", profile: "compact")
  → high-risk files identified
      → find_impact_files(repoId, filePath, view: "surface", profile: "compact")  ← caller blast radius
...
route_map(repoId, profile: "compact")
  → verb + path + handler symbol
      → get_symbol_context_pack(repoId, name)  ← understand handler internals
```
...
MCP tool returns empty or confidence < 0.7
  ...
  → still insufficient → log to `codebase-index-mcp/docs/mcp-codebase-index-issue-registry.md`
