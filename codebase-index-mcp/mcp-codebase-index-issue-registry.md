...
## Session Dogfooding — 2026-06-17 (OSB inbound-notification verification, first `search_regex` dogfood)
Context: MCP-first session on `wec.communication-hub` (branch `develop`, `search_regex` shipped at commit `211a103`). Task: confirm whether the OSB channel raises an agent notification on inbound, then verify against live DB. Flow: `search_symbols(intent)` → `search_regex` (4×, scoping the routing/notify surface) → `get_symbol_source` / `Read` for exact logic → `postgres-mcp` for live-data confirmation. **`search_regex` performed well as a locator** and was the session's primary discovery tool — the per-match `enclosingSymbol` ({symbolId, name, kind}) let me jump straight from a regex hit to `get_symbol_source(symbolId)` without re-reading files (the concrete MCP-over-grep win); `filePathPrefix` + `excludeTests` scoping was clean (e.g. the `IsLogOnly|DispatchSourceType.(Osb|CallLog)` sweep returned exactly 17 production matches across 347 files); alternation patterns worked; `coverage.confidence:"high"` on every call. The decisive evidence (`CommunicationChannels.NotifiesOnInbound`, the `ConversationMessageReceivedEventHandler` gate comment) still came from `get_symbol_source`/`Read` — `search_regex` located, it did not read. The two issues below are where the locate→read handoff cost an avoidable round-trip.

## MCP-ISSUE-026 ✅ RESOLVED (2026-06-17)
- Reported: 2026-06-17 · Repo: `wec.communication-hub` · Branch: `develop` · Commit: `211a103` · Severity: medium (trust — undercuts the new `search_regex` tool's primary output)
- Scenario: Use `search_regex` to locate the OSB-vs-CallLog notification branch, then open the enclosing logic at the reported line.
- MCP tool/query attempted: `search_regex(pattern="Notif|ShouldNotify|notify|DispatchSourceType\\.(Osb|CallLog|Email)", filePathPrefix=...)` and 3 further sweeps, each over the indexed tree.
- Expected: line numbers I can act on directly, or an explicit signal of how far off they may be.
- Actual: **every** `search_regex` (and every `get_symbol_source`) response carried `staleWarning: {note: "index is stale: index commit differs from repo HEAD", hint: "match lines may be off vs current HEAD — re-index for exact positions."}`. The warning is non-actionable: it says *that* the index is stale but not *how much* (no `lastIndexedCommit`/`headCommit`, no commits-behind count, no dirty-file count), so I couldn't tell whether line numbers were off by 0 or 50. I treated all reported lines as untrusted and verified positions via `get_symbol_source(symbolId)` (symbol-relative, stale-resistant) / targeted `Read` on every hit.
- Impact: medium — the headline value of `search_regex` over grep is "match + line + enclosing symbol," but a bare stale flag forces a verifying second call on every match, eroding that advantage. Recurs on any repo whose working tree has drifted since the last index (i.e. most active sessions).
- Workaround: re-index first (`index_repository(mode="dirty")`), or ignore reported lines and re-anchor through `get_symbol_source(symbolId)`.
- Enhancement proposal: make the stale signal actionable — (1) include `lastIndexedCommit`, `headCommit`, `commitsBehind`, and `dirtyCount` in `staleWarning` so the caller can size the risk and decide whether to re-index; (2) optionally return a stable anchor alongside each match — `enclosingSymbolId` + **offset-within-symbol** (line delta from the symbol's start, which survives edits above the symbol) rather than only an absolute file line; (3) consider auto-running a `dirty` re-index when `dirtyCount` is small (cheap) before serving the query, or surface a one-line `suggestion: "run index_repository(mode='dirty')"` exactly like ISSUE-019's strategy hint. Pairs with ENH-A (dirty freshness) but applies it to the new `search_regex`/`get_symbol_source` lane.
- Tracking URL: `D:/1.SourceCode/mcp-local/codebase-index-mcp/mcp-codebase-index-issue-registry.md#mcp-issue-026`

## MCP-ISSUE-027 ✅ RESOLVED (2026-06-17)
- Reported: 2026-06-17 · Repo: `wec.communication-hub` · Branch: `develop` · Commit: `211a103` · Severity: medium (forces a guaranteed second call per useful hit)
- Scenario: After `search_regex` pinpoints the branch, read the few lines around the match to decide whether the hit is the logic I want — without a full `get_symbol_source`/`Read` round-trip for matches that turn out irrelevant.
- MCP tool/query attempted: `search_regex(..., contextLines=2)` and `contextLines=3` on the InboundMessageConsumer / ProcessInboundMessage sweeps.
- Expected: the requested surrounding source lines in the response (that is what `contextLines` advertises).
- Actual: each match returned only `{ filePath, line, matchText, enclosingSymbol }` — `matchText` is the **single** matching line; the `contextLines=2/3` I passed produced **no** surrounding lines in the response (default profile). So `search_regex` tells me *where* and *which symbol*, but to see even ±2 lines I must issue `get_symbol_source(symbolId)` or `Read`. For a sweep that returns many candidate hits, that is one mandatory follow-up call per hit I want to triage.
- Impact: medium — the locate→read handoff that should be one call (regex with context) becomes two (regex, then source). On this task it was tolerable (few hits), but for a broad TODO/call-site sweep it multiplies calls against the per-question budget and reads as "grep that also costs a second fetch."
- Workaround: skip `contextLines`, go straight to `get_symbol_source(symbolId)` for every hit worth inspecting.
- Enhancement proposal: actually return the `contextLines` window per match (`before[]`/`after[]` or a small `snippet`), at least in `standard`/`verbose` profiles — if it is profile-gated today, document that and surface a short snippet in `compact` too (a few lines is cheap and is the whole point of the param). If `contextLines` is silently ignored under the default profile, treat that as a bug: either honor it or reject the param. This converts the dominant `search_regex` → `get_symbol_source` round-trip into a single call for triage-grade reads.
- Tracking URL: `D:/1.SourceCode/mcp-local/codebase-index-mcp/mcp-codebase-index-issue-registry.md#mcp-issue-027`

## MCP-ISSUE-028 ✅ RESOLVED (2026-06-17)
- Reported: 2026-06-17 · Repo: `wec.communication-hub` · Branch: `develop` · Commit: `211a103` · Severity: low (efficiency — observed, not blocking this session)
- Scenario: A notify/routing branch in this codebase straddles two layers — `Application/Conversations/Commands/...` (routing decision) and `Infrastructure/Messaging/Consumers/...` (consumer entry). I wanted one `search_regex` sweep scoped to *both* those subtrees while still excluding the rest of the repo.
- MCP tool/query attempted: `search_regex(pattern=..., filePathPrefix="backend/.../Commands/...")` — `filePathPrefix` accepts a **single** string.
- Expected: scope a sweep to a set of paths (`["…/Commands/…", "…/Consumers/…"]`) in one call, or fall back to a single broad prefix.
- Actual: with one prefix I either ran two separate `search_regex` calls (one per subtree) or widened to a common ancestor prefix (`backend/CommunicationHub/src`) and accepted matches from unrelated folders. This session I used the broad-prefix path; it worked but pulled in noise I had to scan past, and a two-layer feature would otherwise cost two calls against the per-question soft cap.
- Impact: low — single-prefix + `excludeTests` covers the common case well; only multi-subtree sweeps (cross-layer features, which are common in Clean Architecture) pay a small extra-call or extra-noise cost.
- Workaround: issue one `search_regex` per subtree, or use a broad common-ancestor prefix and filter the results by eye.
- Enhancement proposal: accept `filePathPrefix` as a **string or string[]** (OR-semantics across prefixes), mirroring how a glob list would scope it; keep the single-string form working unchanged. Optionally add a `pathExclude` glob (parallels the `excludeTests` precedent and ISSUE-024's suggested `pathExclude`) so a sweep can subtract a noisy subtree instead of having to enumerate includes. Pure query-scoping change, no extraction work.
- Tracking URL: `D:/1.SourceCode/mcp-local/codebase-index-mcp/mcp-codebase-index-issue-registry.md#mcp-issue-028`

### Adoption-lift priority (this session)
1. **ISSUE-026** (actionable stale signal) — highest leverage: it makes `search_regex` line numbers trustworthy (or honestly sized), removing the verifying second call that currently follows every match. Cheap (telemetry already knows the commit/dirty state).
2. **ISSUE-027** (return `contextLines` window) — collapses the routine `search_regex` → `get_symbol_source` triage round-trip into one call; pure read/response-shape change, no extraction work.
3. **ISSUE-028** (multi-prefix / `pathExclude` scoping) — lowest priority, cheap quality-of-life: lets a cross-layer sweep stay one focused call instead of two-calls-or-broaden-and-filter.

All three are response/query-layer refinements over the freshly-shipped `search_regex` (no re-extraction needed); together they close the gap between "located the code" and "can act on it" that this session paid for on every hit.

### Resolution (2026-06-17) — ISSUE-026/027/028 all shipped
- **ISSUE-026 — actionable stale signal.** The shared `buildStaleWarning` (`src/gitHelpers.ts`) now returns `{ note, hint, lastIndexedCommit, headCommit, commitsBehind, dirtyCount }` instead of bare `{ note, hint }`, so a caller can size the drift and decide whether to re-index. Composed from already-tested helpers (`getRepoStaleness` for the two short shas, `countCommitsBehind`, `collectDirtyFiles`) — the extra git lookups run at most once per response (only on the stale path), never per match. Enriching the shared helper lifts **every** consumer at once (`search_regex`, `search_literals`, `get_symbol_source`, and the impact tools via `staleWarningFor`). `search_regex`'s hint was also corrected: its match lines/text are read **live from disk** and are always current — only `enclosingSymbol` (resolved from indexed line-ranges via `findSymbolAtLine`) can drift, so the hint now points re-index value at enclosing-symbol accuracy rather than line positions. Residual: still fires only on commit drift (HEAD ≠ indexed commit); a dirty-tree-only tree at the same HEAD does not raise the warning (left as a follow-up to avoid a behavior change across all consumers).
- **ISSUE-027 — context window in `compact`.** `searchRegexImpl` already computed `beforeContext`/`afterContext` per match and `standard`/`verbose` already returned them; only the default `compact` profile dropped them. `handleSearchRegex` (`src/handlers/searchHandler.ts`) now includes `beforeContext`/`afterContext` in the compact match shape when `contextLines > 0` (and omits them at `contextLines:0`), collapsing the routine `search_regex` → `get_symbol_source` triage round-trip into one call. `nano` stays minimal. Safe re: the `benchmark:plan:check` gate — `search_regex` is not a benchmark scenario (verified: no snapshot regression).
- **ISSUE-028 — multi-prefix + `pathExclude`.** `filePathPrefix` now accepts a **string or string[]** (OR-semantics: in scope if it starts with any prefix); single-string form unchanged. New `pathExclude` (string or string[]) subtracts **minimatch globs** (e.g. `**/Tests/**`, `*.generated.cs`), matched case-insensitively with `dot:true` against the normalized POSIX path. So a cross-layer Clean-Architecture sweep stays one scoped call. `minimatch` promoted to an explicit dependency (was already present transitively via `glob` v11). Changes span `src/regexSearch.ts` (filter logic + `toArray` normalizer), `RegexSearchOptions`, the Zod schema (`src/schemas/toolSchemas.ts`), and the hand-written JSON `inputSchema` + tool description (`src/index.ts`). Pure query-scoping change, no extraction work.
- Verification (2026-06-17): `npm run typecheck` ✅ · `npm run build` ✅ · `npm run guard:no-llm-runtime` ✅ · `npm run test:search-regex` ✅ (new COMPACT_CONTEXT_OK / MULTI_PREFIX_OK / PATH_EXCLUDE_OK cases) · `node scripts/smoke-test.mjs` ✅ (new SEARCH_REGEX_CONTEXT_OK + SEARCH_REGEX_SCOPE_OK; note: smoke-test has a **pre-existing intermittent index-phase server crash** on Windows — reproduces on clean `main` ~50% of runs, unrelated to these changes; a completed run passes including the new assertions) · `npm run benchmark:plan:check` ✅ (no snapshot regression).

## MCP-ISSUE-029 ✅ RESOLVED (2026-06-19)
- Resolution (2026-06-19, PR1 `fix/mcp-issue-029-backreference`): **Primary corruption fixed.** Root cause was a single line — `buildRefactorPreview` correctly expanded `$1`/`$&` into `hunk.afterText` (`refactorEngine.ts:171`), but `handlers/refactorHandler.ts:176` persisted the hunk with `replacementText: args.replaceExpression` (the raw template), and apply writes `replacementText` (`refactorEngine.ts:572`) → literal `$1` on disk. Fix: `replacementText: hunk.afterText` (mirrors the already-correct `refactor_symbol_migration` at :420; the approval digest already hashes `afterText`, so no token breakage). Also: (a) `expandRegexReplacement` now substitutes named groups (`$<name>`/`${name}`) in addition to `$1`..`$99`/`$&`/`$$`; (b) new `unsubstituted_backreference` risk flag — a backreference to a group that did not match is flagged at preview (diagnostics `PREVIEW_HAS_UNSUBSTITUTED_BACKREFERENCE`) and **blocked at apply** (`RISK_FLAG_BLOCKED`), so a stray backreference is never silently written; (c) tool description + README updated to document the supported backreference syntax.
- Resolution (2026-06-19, PR2, secondary parallel-apply defect): applies now run under an in-process mutex (`refactorHandler.ts` `runExclusiveApply`) covering the read-modify-write **and** its persistence, so concurrent applies serialize deterministically instead of interleaving. When a preview is genuinely invalidated by a prior apply of the same file (current on-disk hash == that apply's `fileHashAfter`, via new `store.getRecentAppliedFileHashes`), the conflict is reported as `FILE_CHANGED_BY_CONCURRENT_APPLY` (actionable) rather than the bare `FILE_CHANGED_AFTER_PREVIEW`.
- Regression coverage: `scripts/test-refactor-engine.mjs` (now registered as `npm run test:refactor-engine`) cases 3.13 (capture-group apply round-trip on disk), 3.14 (named-group), 3.15 (guard-block), 3.16 (concurrent-apply diagnostic) — **65/65 pass**; full pre-commit gate green (typecheck, build, no-llm guard, smoke, benchmark). Also fixed a latent test-harness deadlock: the suite did not drain the server's piped stderr, so a chatty server could fill the OS pipe buffer and block its own event loop (`transport.stderr?.resume()`).
- Reported: 2026-06-19 · Repo: `wec.communication-hub` · Branch: `refactor/domain-enum-consistency` (work branch; registry committed from `develop` @ `6dde9e3`) · Severity: **high** (silent file corruption → guaranteed build break; the tool wrote invalid source and reported success)
- Scenario: Bulk-convert ~33 test assertions of the form `Assert.Equal("ai", conv.AssignmentState.HandledBy)` → `Assert.Equal(ConversationHandledByValues.Ai, conv.AssignmentState.HandledBy)` during the `handled_by` string→enum promotion (WS5). Receiver expression (`conv`, `result.Conversation`, etc.) varies per call-site, so I used a capture group to preserve it: `findMode:"regex"`, pattern `Equal\("ai", ([^)]+\.AssignmentState\.HandledBy)\)`, `replaceExpression` `Equal(ConversationHandledByValues.Ai, $1)`.
- MCP tool/query attempted: `refactor_replace_preview` → `refactor_replace_apply` across 8 test files (`ProcessInboundMessageCommandHandlerTests.cs`, `ReplyConversationCommandHandlerTests.cs`, `ConversationEntityTests.cs`, et al.).
- Expected: `$1` (and `$&`) in `replaceExpression` substitute the captured group / whole match, as the tool description for regex mode advertises — yielding valid C# with the original receiver preserved.
- Actual: **`$1`/`$&` are NOT substituted** — the tool wrote the **literal characters** `$1` into each file (`Assert.Equal(ConversationHandledByValues.Ai, $1)`). The preview's `replacementText` also showed the literal `$1`, but `confidence`/`riskFlags` were clean and `apply` reported success — no warning that the replacement contained an un-substituted backreference. Build then failed with **99 × `CS1056 Unexpected character '$'`**. The 38 *initializer* edits in the same batch (no capture group — plain literal `"ai"` → `ConversationHandledByValues.Ai`) applied correctly; only the 33 capture-group assertions were corrupted.
- Secondary defect observed in recovery: issuing `refactor_replace_apply` in **parallel** across files whose previews overlapped failed with `FILE_CHANGED_AFTER_PREVIEW` — the first apply shifts line numbers and silently invalidates every other in-flight preview of the same file. Applies must be serialized preview→apply, one op at a time.
- Impact: high — capture-group substitution is the natural way to do context-preserving bulk rewrites (the exact case `refactor_replace` exists for). When it silently emits literal `$1` yet reports success, it corrupts every matched site and the only signal is a downstream compiler error. Anyone reaching for `refactor_replace` over `perl`/`sed` *because it is "validated and safer"* (per CLAUDE.md MCP-first guidance) is the most exposed.
- Workaround (verified working this session): avoid capture groups entirely. Match only the substring being changed and constrain context with a **zero-width lookahead** (lookahead IS supported): find `"ai", (?=[^;\n]*?\.AssignmentState\.HandledBy)` → replace `ConversationHandledByValues.Ai, ` — no backreference needed, receiver untouched. Recovery: `refactor_replace_rollback` on the 3 broken assertion applies (atomic, 0 conflicts, kept the 38 good initializer edits), then re-ran with the lookahead form. Serialize applies.
- Enhancement proposal: (1) actually substitute `$1`/`$2`/`$&`/`${name}` in `replaceExpression` under `findMode:"regex"` (standard JS `String.replace` backreference semantics); (2) **failing that, until it works**, add a preview-stage guard that detects an un-substituted backreference token (`$1`, `$&`, …) surviving into `replacementText` and raises a `riskFlag`/blocks apply (`UNSUBSTITUTED_BACKREFERENCE`) — the same defensive posture ISSUE-003 took for invalid C# initializers, so the tool never silently writes literal `$1`; (3) document explicitly whether capture-group substitution is supported, and if not, point users to the lookahead workaround in the tool description; (4) make `apply` reject (not silently shift) when a sibling preview for the same file is still pending, with a clearer message than `FILE_CHANGED_AFTER_PREVIEW`, or auto-serialize a batch.
- Tracking URL: `D:/1.SourceCode/mcp-local/codebase-index-mcp/mcp-codebase-index-issue-registry.md#mcp-issue-029`

## ENH-2026-06-19 — Enhancements from the enum-persistence refactor session
Context: `wec.communication-hub` · branch `refactor/domain-enum-consistency` · registry committed from `develop` @ `6dde9e3`. These are **enhancements** (capability gaps), not bugs — distinct from ISSUE-029. All grounded in real friction during WS1–WS6 (unify every Conversation enum on snake_case string storage; promote `handled_by` string→`ConversationHandledBy` enum). Ordered by leverage.

- **ENH-029-A — ✅ IMPLEMENTED (2026-06-22).** New `change_value_representation` tool (`src/valueRepresentation.ts` + handler `handleChangeValueRepresentation`): given `{ property, requiredOwnerType, valueMap }`, it locates every literal site of the property via the **C# AST** — assignments (`conv.HandledBy = "ai"`), object initializers (`new Conversation { HandledBy = "human" }`), `==`/`!=` comparisons, and assertion arguments (`Assert.Equal("ai", x.HandledBy)`) — and rewrites the literal token to the mapped enum member. No user-authored regex/backreference, so it sidesteps the MCP-ISSUE-029 capture-group failure mode entirely. Owner type is verified per site (object-creation type for initializers; receiver type for member access via local/param/field type inference, with `var x = new T()` resolved from the initializer); **cross-type sites — a same-named property on a different owner — are skipped, never rewritten**, and unprovable sites are flagged `ambiguous_target` (low confidence, blocked from apply unless `includeLowConfidence`). Reuses the existing preview→apply→rollback pipeline (HMAC token, digest, parallel-apply mutex), so it is preview-gated and reversible for free. `includeComparisons:false` narrows to write-sites only. Test: `npm run test:change-value-representation` (10/10, incl. cross-type skip + rollback). This is the exact work that forced the regex gymnastics in WS1–WS6. Original proposal below.
- **ENH-029-A — Primitive for "promote property literal → enum" (highest).** The whole session was one recurring structured refactor: change a property's type (string→enum) → update the Constants converter → update the EF `HasConversion` → rewrite every call-site literal (`HandledBy = "ai"` → `…Values.Ai`; `Equal("ai", x.HandledBy)` → `Equal(…Values.Ai, …)`). `refactor_symbol_migration` understands **member-path** migration (`CrmCustomerId → IdentityState.CrmCustomerId`) but NOT **value-literal → enum-member** rewrites, so I dropped to hand-written regex + capture groups and hit ISSUE-029. Proposal: a `change_value_representation` / enum-promotion assist — given a property + a value map (`{"ai": ConversationHandledBy.Ai, "human": …Human}`), it finds every literal assignment/comparison of that property and rewrites to the enum member via AST (no user-authored backreference). This is the exact work that forced the regex gymnastics.
- **ENH-029-B — ✅ IMPLEMENTED (2026-06-19).** `find_field_accesses` now returns `assignedExpression` (the RHS source text) per PROPERTY_WRITE site, for both member-access writes (`conv.HandledBy = "ai"`) and object-initializer writes (`new Conversation { HandledBy = "human" }`). Capture path: C# extractor grabs the assignment RHS via `getAssignedExpressionText`/`truncateAssignedExpression` (200-char cap), threaded through `emitNestedPropertyEdges`/`emitPropertyAccessEdge`; stored in a new additive nullable `edges.assigned_expression` column (`ensureEdgeColumn` migration) that `resolvePropertyEdges`' in-place UPDATE never touches, so it survives resolution. Critical: distinct RHS values must survive **two** dedup layers — `dedupeEdges` (extractor) and `deduplicateResolvedEdges` (post-resolve `GROUP BY`); both keys now include `assigned_expression`, so `"ai"` and `"human"` at the same enclosing symbol stay separate rows. Surfaced in `getFieldAccesses` (standard + compact). Test: `npm run test:field-access-rhs` (3/3). Feeds ENH-029-A. Original proposal below.
- **ENH-029-B — `find_field_accesses` should return the assigned literal/RHS at write-sites.** Before changing `HandledBy`'s type I needed the **value domain** actually assigned (`"ai"`/`"human"`) to build the enum map. The tool returns enclosing symbol but not the right-hand expression, forcing a second `search_regex` sweep. Proposal: in `mode:write|all`, include `assignedExpression`/`literalValue` per write-site — one call confirms the value-domain before the type change, and it feeds directly into ENH-029-A.
- **ENH-029-C — ✅ IMPLEMENTED (2026-06-22).** New `get_persistence_mapping(property, ownerType?)` tool (`src/efPersistence.ts` + handler `handleGetPersistenceMapping`). On demand (no new pipeline lane — a targeted per-property lookup), it parses EF configuration files (`**/Configurations/**`, `*Configuration.cs`/`*Config.cs`, `*DbContext*`) with tree-sitter and returns per-property `{ columnName, hasConverter, converterExpression, maxLength }` (owner type resolved from `IEntityTypeConfiguration<T>`) plus associated `HasCheckConstraint` rows (linked by column name). When the property is value-converted it also scans query files for the **EF projection trap**: a member-access of the property inside an EF-translated `.Select()`/`.Where()` lambda whose receiver spine is not materialized (`.ToListAsync()`/`.AsEnumerable()`/etc.) → `DB_TRANSLATED_PROJECTION` warning. Includes lightweight intra-method dataflow so `var items = await q.ToListAsync(); items.Select(…)` is correctly treated as in-memory (not flagged). Bounded to 800 files scanned with a substring pre-filter. Test: `npm run test:persistence-mapping` (10/10). Original proposal below.
- **ENH-029-C — EF/persistence-aware lane (high; specific to this EF-heavy codebase).** The risky layer wasn't C# symbols, it was persistence: is a converter store-shape-neutral, the CHECK constraint, and especially the **EF projection trap**. At `GetCustomerConversationDetail` I had to reason that a converted enum only materializes the CLR enum *after* `.ToListAsync()`, so `ToStorageValue(...)` must run in-memory, NOT inside a `.Select()` EF translates to SQL — invisible to a symbol graph. Proposal: for a given property, return (a) its EF mapping (column, converter, maxLength, CHECK) and (b) a warning if it appears in a `.Select()`/`.Where()` that EF translates (flag `DB_TRANSLATED_PROJECTION`). Recurring hazard wherever `HasConversion` is used.
- **ENH-029-D — ✅ IMPLEMENTED (2026-06-19).** `linkTestsToSource` (`src/staticAnalyzer.ts`) now detects a persistence-layer change (`isInfraPersistencePath`: `Migrations/`, `Data/Configurations/`, `Persistence/`, `DbContext/`, `Infrastructure/Data/`) and (a) admits integration/system/e2e tests (`isIntegrationTestPath`) into selection on path even when names don't overlap, and (b) boosts the integration-test↔changed-file link by 0.6 with reason `infrastructure_integration_priority` so the round-trip test ranks into `testsToRun`. Test: `npm run test:link-tests-boost` (3/3). Original proposal below.
- **ENH-029-D — `change_impact` should boost integration tests for EF-config/migration changes.** When I changed a converter, the test that actually validated it was the Postgres round-trip (`InboundMessageConsumerIntegrationTests`), not a same-named unit test — but name-affinity test linkage favors unit tests. Proposal: when a changed file is under `Infrastructure/Data/Configurations/**` or `Migrations/**`, boost `Infrastructure.IntegrationTests` in `testsToRun`, since that layer is the only one that proves the converter/CHECK round-trip.
- **ENH-029-E — ✅ IMPLEMENTED (2026-06-22).** New `get_value_contract_impact(value, column?, repoIds?)` tool (`src/valueContract.ts` + handler `handleGetValueContractImpact`). Fans the existing `search_literals` over every registered repo, keeps **exact-value** hits (the contract is the literal itself), reads each hit's source line, and classifies it producer (single-`=` assignment / SQL INSERT·SET / `return`) vs consumer (`==`/`!=`/`WHERE`/`case`/`.Equals(`) — sharpened by the optional `column`. Groups by repo, ranked by hit count, with per-repo producer/consumer/unknown tallies. Answers "who across all repos reads/writes this stored value" — the WS0 data-contract gate that `get_cross_repo_impact` (symbol-oriented) cannot. Bounded to 500 hits/repo. Test: `npm run test:value-contract-impact` (5/5, producer vs consumer across two repos). Original proposal below.
- **ENH-029-E — Cross-repo impact at the storage/wire-VALUE level, not just symbols (for the WS0 gate).** WS0 required proving no external repo reads `conversations.status` as an **int** before migrating it to string. The blast radius here isn't a code symbol — it's the **stored/wire value** (`5`, `"resolved"`). `get_cross_repo_impact` is symbol-oriented and can't answer "who reads this DB column / this magic value across repos." Proposal: a "value/literal contract" cross-repo lane that traces where a storage token (`"resolved"`, status int `5`) flows across repos — the only thing that genuinely gates a data-contract migration.

Note: ENH-029-C and -D are highest-value **because this is an EF-heavy codebase** — they cover the persistence layer that cost the most reasoning and that the symbol graph can't see today. ENH-029-A/-B are more generic but pay off on the next enum promotion.

**Status (2026-06-22): all five enhancements implemented.** ENH-029-A `change_value_representation`, ENH-029-B `find_field_accesses.assignedExpression`, ENH-029-C `get_persistence_mapping`, ENH-029-D integration-test boost, ENH-029-E `get_value_contract_impact`. New test scripts: `test:change-value-representation`, `test:field-access-rhs`, `test:persistence-mapping`, `test:value-contract-impact`, `test:link-tests-boost`. All rule/AST-based (no-LLM guard green). Shipped alongside the MCP-ISSUE-029 core fix (PR1) + parallel-apply hardening (PR2).

**Code review hardening (2026-06-22).** A recall-biased review surfaced five findings, all fixed:
1. **Parallel-apply hardening was incomplete** — `refactor_symbol_migration`'s apply bypassed the new apply mutex + `recentAppliedHashByFile`, so it neither serialized with the other apply paths nor emitted `FILE_CHANGED_BY_CONCURRENT_APPLY`. Root cause: the apply block was copy-pasted across three handlers. Fixed by extracting a shared `applyPreviewExclusively` helper used by all three apply paths (`refactor_replace`, `refactor_symbol_migration`, `change_value_representation`). `handleRefactorSymbolMigration` is now async; the dispatch `await`s it so policy errors (e.g. `INVALID_INITIALIZER_REWRITE`) still map to an isError result.
2. **`unsubstituted_backreference` false positive** — a reference to a *declared but non-participating* optional group (`(x)?…$1` where `match[n] === undefined`) was flagged and blocked, though substituting empty is correct `String.replace` behavior. `hasMissingBackreference` now flags only references the pattern does not declare at all (numbered: `n >= match.length`; named: `!(name in match.groups)`). New regression case 3.17 in `test-refactor-engine.mjs` (now 69/69).
3. **Non-deterministic concurrent-apply detection** — `getRecentAppliedFileHashesImpl` tie-broke same-millisecond applies on a random `apply_id`; now orders by `a.rowid desc` (true insertion order).
4. **Read tools ignored the response profile** — `get_persistence_mapping` / `get_value_contract_impact` defaulted to `standard` instead of the documented `compact`; both now accept a `profile` arg (default `compact`); `change_value_representation` accepts one too (default `standard`, matching sibling refactor tools).
5. Apply/preview duplication removed via the shared helper in (1). Full gate green (typecheck, build, no-LLM guard, smoke, benchmark; refactor-engine 69/69, all ENH suites pass).

## MCP-ISSUE-030 ✅ RESOLVED (2026-06-22) — on-demand parse threw "Invalid argument" on files >32KB
- Severity: **high** — `change_value_representation` (ENH-029-A) and `get_persistence_mapping` (ENH-029-C) threw `INTERNAL_ERROR: "Invalid argument"` and returned **zero** results on any repo containing a C# file larger than **32768 bytes (32KB)**. On a real codebase this is most files, so both tools were effectively unusable live despite their unit suites passing.
- Root cause: node-tree-sitter's native `parser.parse(source)` throws `"Invalid argument"` when `source` exceeds the **default 32KB buffer** unless an explicit `{ bufferSize }` is passed. The index pipeline (`treeSitterExtractor.extractGraphData`) already passed `bufferSize: getTreeSitterBufferSize(sourceBytes)` (line 115-117), but the two **on-demand** parse paths added in ENH-029-A/-C did not — they called the bare `parser.parse(content)` / `getParser().parse(content)`. Empirically confirmed via probe: source of 32056 bytes parses OK, 33056 bytes throws `"Invalid argument"`; the threshold is exactly 32768.
- Why it slipped past the original ENH suites: `test:change-value-representation` and `test:persistence-mapping` used small fixtures (<1KB), which never crossed the 32KB native-buffer limit, so the defect was invisible to the tests while the tools shipped marked ✅.
- Fix: pass `{ bufferSize: getTreeSitterBufferSize(Buffer.byteLength(content, "utf8")) }` at every on-demand parse site, mirroring the index pipeline. `getTreeSitterBufferSize` exported from `treeSitterExtractor.ts` (was private) and imported by both consumers.
  - `src/valueRepresentation.ts:271` — `parser.parse(content, undefined, { bufferSize: … })`
  - `src/efPersistence.ts` — both parse sites (`extractMappingsFromFile` Pass 1 + `findProjectionTrapsInFile` Pass 2).
- Regression coverage (the real fix): both ENH suites now **pad their fixtures past 32KB** so they exercise the bufferSize path — without the fix every assertion fails on `"Invalid argument"`. Critical subtlety: the pad must be **many short lines**, not one giant single-line comment. The indexer's `fileFilter.isLikelyMinified` (samples first 10KB, skips the file if avg line length > 500) silently excludes a file padded with one 40KB-line comment (`filesIndexed=0`, no parse failure) — so the tool, which scans only `listIndexedFiles`, never sees it. The padded fixtures use ~700 short comment lines (avg ~70 chars) → >32KB **and** not flagged minified. `test:change-value-representation` 10/10, `test:persistence-mapping` 10/10.
- Live verification (2026-06-22, repo `wec.communication-hub`, post-rebuild + MCP restart): `get_persistence_mapping(HandledBy, Conversation)` → full mapping (`column=handled_by`, `hasConverter`, `maxLength=16`, CHECK `handled_by IN ('ai','human')`) + correct `DB_TRANSLATED_PROJECTION` at `GetCustomerConversationDetail.cs:87`, `filesScanned:12`, no error. `change_value_representation(HandledBy, Conversation, dryRun)` → parsed all large files, returned a valid preview (3 matches, correctly flagged `ambiguous_target` because they sit in a test file where the owner type can't be proven). Both tools no longer throw.
- Lesson for the registry: any **on-demand** tree-sitter parse added outside the index pipeline must pass `bufferSize`, and its regression fixture must exceed 32KB **with realistic line lengths** to actually cover the native-buffer path (a small fixture or a single-long-line pad both give false-green).
- Tracking URL: `D:/1.SourceCode/mcp-local/codebase-index-mcp/mcp-codebase-index-issue-registry.md#mcp-issue-030`
