...
New here? [Onboarding](onboarding.md) is the three-command version. This page is what you keep open
afterwards.
...
([ADR 0001](adr/0001-workspace-native-deps.md)).
...
Packages use `build`, `typecheck`, `test`, `clean`. Add a server to
`packages/manifest/src/servers.ts` and every aggregate picks it up — nothing else needs editing.
...
stays advisory ([Folder Convention](folder-convention.md) §5).
...
- [Onboarding](onboarding.md) · [Contribution Guide](../CONTRIBUTING.md)
- [Server Development Guide](server-development.md) · [Tool Development Guide](tool-development.md)
- [Conventions](conventions.md) — every rule, sorted by what enforces it
- `docs/migration/ci.md` — what CI covers and what it deliberately does not
