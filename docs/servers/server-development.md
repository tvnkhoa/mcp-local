...
No `.gitignore` — the root one covers every server ([ADR 0003](../decisions/0003-single-root-gitignore.md)).
Only the slots with content are created; add `services/`, `repositories/`, `resources/`, `types/` as
the server grows ([Folder Convention](../reference/folder-convention.md) §2).

> **Do not hand-build a server.** §1–2 of this guide supersede the `mcp-scaffold` authoring skill,
> archived 2026-08-03 because it had drifted: it prescribed a root guardrails file (`sqlGuardrails.ts`)
...
| S5 | `<server>/scripts/smoke-test.mjs` does a real stdio handshake and lists tools |
...
```bash
npm run test               # unit first, then the 32 integration harnesses, one result
...
See [Development Guide](../development/workflow.md) for what that covers, what CI covers, and where the two
differ.
...
- [Tool Development Guide](./tool-development.md) · [Development Guide](../development/workflow.md)
- [Folder Convention](../reference/folder-convention.md) · [Dependency Rules](../reference/dependency-rules.md)
- [Package Overview](../reference/packages.md) — what `@mcp/sdk` and `@mcp/shared` give you
- `templates/server/README.md` — the scaffold's own instructions
- `packages/manifest/README.md` — the manifest reference
- `.claude/skills/mcp-skill-authoring/` · `mcp-security-review/` · `mcp-contract-conformance/`
