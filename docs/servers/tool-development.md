...
Clients use these hints to decide what may be auto-approved; the current inventory of non-read-only
and destructive tools is in [`../../contracts/README.md`](../../contracts/README.md). **A wrong one is a safety bug, not a
documentation bug.**
...
The classes are **passed in, never imported by the SDK**. Per [ADR 0001](../decisions/0001-workspace-native-deps.md)
...
- [Package Overview](../reference/packages.md) — `@mcp/sdk`'s full surface
