...
A guard that has never been shown to fail is a guard on trust. Each was given a deliberate
violation, confirmed to reject it, and reverted — re-runnable via `scripts/prove-guards.sh`:

| Rule | Violation injected | Result |
|---|---|---|
| `env/direct-access` | `process.env.SNEAKY_VALUE` in `postgres-mcp/src/index.ts` | rejected |
| `servers/cross-import` | `observe-mcp` importing `codebase-index-mcp`'s graph store | rejected |
| `servers/tooling-import` | `bitbucket-mcp` importing `@mcp/manifest` | rejected |
...
- [ADR 0001](adr/0001-workspace-native-deps.md) — why servers stay outside the workspace
