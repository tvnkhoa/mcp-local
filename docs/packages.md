...
deliberately outside it ([ADR 0001](adr/0001-workspace-native-deps.md)) and consume these through
`file:` dependencies.
...
|---|---|---|---|---|
| 0 | `@mcp/core` | Zero-dependency primitives: `Result`, error taxonomy, env reader, logger, redaction, profiles, limits, paths | *(nothing)* | `.` |
| 1 | `@mcp/sdk` | The tool builder and the MCP runtime. **The only importer of `@modelcontextprotocol/sdk`** | `@mcp/core`, `zod`, protocol SDK | `.` |
| 2 | `@mcp/shared` | Capabilities: approval tokens, SQL guardrails, HTTP client, path allowlist | `@mcp/core` | `.`, `./approval`, `./sql`, `./http`, `./fs` |
| 3 | `@mcp/testing` | Harness for invoking tools without a server, leak assertions, contract snapshots | `@mcp/core`, `@mcp/sdk` | `.` |
| 4 | `@mcp/cli` | The architecture guards and the `mcp-platform` command | `@mcp/core` | `.` |
| 5 | `@mcp/manifest` | Workspace tooling data: which servers exist, their entry points, their env contracts | `@mcp/core` | `.` |
...
fork ([ADR 0002](adr/0002-sql-guardrail-token-lists.md)).
...
- [Tool Development Guide](tool-development.md) — using `@mcp/sdk` in anger
- `packages/<name>/README.md` — the per-package reference
- `docs/migration/foundation-notes.md` — what the foundation contains and why
- `docs/refactor/duplication-extraction-report.md` — the extraction, its measured behaviour deltas,
