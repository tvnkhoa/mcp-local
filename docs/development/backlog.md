...
day — the reasoning is in the item and in `docs/development/ci.md`.
...
**Input** — the post-migration assessment of `docs/archive/migration/status.md` §, re-measured against the
...
condition, and it is what would have to change to reopen this. See `docs/development/ci.md`
...
**Why**
CI is credential-free by design (S-05, `docs/development/ci.md`) and that decision is right. The
...
- `docs/development/ci.md` — state what the second workflow covers and what it still does not
...
§9 said 89, `CLAUDE.md` and `docs/architecture/as-built.md` said 94, the manifest reports **96** — all four
now agree.
...
- `docs/archive/migration/README.md` and `CLAUDE.md` now point at `status.md` for state and describe
...
**Files affected** `docs/archive/migration/status.md`, `docs/archive/migration/migration-plan.md` (header only),
...
The three prose copies (`codebase-index-mcp/CLAUDE.md`, `docs/guides/onboarding.md`,
`docs/reference/conventions.md` §9) now point at the check instead of only warning — they stay because the
...
`docs/guides/onboarding.md` and `docs/reference/conventions.md` §9 — three copies of a rule nothing enforces, which
...
| Live backends never reached from CI | No credential is copied into GitHub. One operator, one machine — every copy is another place to leak from and another rotation obligation. `verify:live` is a local release step, and the risk of a client path rotting unnoticed is accepted | B-05 won't-do · `docs/development/ci.md` |
