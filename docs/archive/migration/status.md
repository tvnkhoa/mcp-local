...
`docs/archive/migration/s26-s29-plan.md` uses the commit-side numbering throughout. Its "S-26" is
...
|---|---|---|
| S-01 Capture the pre-migration baseline | ✅ | `docs/archive/audit-report.md` (audit of `01c532e`) |
...
| Step | Status | Evidence |
|---|---|---|
| S-07 Native dependency spike (throwaway) | ✅ | decision implemented long ago; record written 2026-07-28 as `docs/decisions/0001-workspace-native-deps.md` |
...
| S-18 Reconcile guardrail token lists | ✅ | `docs/decisions/0002-sql-guardrail-token-lists.md` — **decision: no change**; pinned by two tests |
...
| S-23 Pilot: `bitbucket-mcp` onto the SDK | ✅ | `docs/archive/migration/s06-s23-notes.md` |
...
| Step | Status | Evidence |
|---|---|---|
| S-37 Normalize folder layout + per-server `.gitignore` | ✅ | `3f5b702`; 48 files, `docs/archive/migration/normalization-report.md`. **Decision: no per-server `.gitignore`** — `docs/decisions/0003-single-root-gitignore.md`. Re-homing folded into S-41 by agreement |
...
2. **Stale `dist/`.** `tsc` does not prune. After the re-home, the probe still held hardcoded
   ...
   `dist/` and rebuilding. Now warned about in `codebase-index-mcp/CLAUDE.md`, `docs/guides/onboarding.md`
   and `docs/reference/conventions.md` §9.
...
- `docs/archive/migration/normalization-report.md` records that those files were **already moved once**,
...
|---|---|---|
| Standard `src/` structure in all four servers | ✅ | `d692094` + `7676dbd` — 153 files moved, 1 split; full report in `docs/archive/refactor/standard-structure-report.md` |
