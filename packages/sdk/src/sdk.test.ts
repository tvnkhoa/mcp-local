...
test("registry: a legacy descriptor without annotations is published unchanged", () => {
  // The reason `ListedToolDescriptor` makes `annotations` optional. A legacy tool predates the
  // requirement, so its descriptor is whatever the server already publishes — codebase-index-mcp
  // has 43 tools with no annotations and a committed snapshot to match. If the bridge added them,
  // installing it would silently rewrite the public contract of every unmigrated tool.
  const legacy: LegacyBridge = {
    listTools: () => [{ name: "annotation_free", description: "as published", inputSchema: EMPTY_OBJECT_SCHEMA }],
    has: (name) => name === "annotation_free",
    call: async () => asText({ from: "legacy" }, "compact")
  };

  const listed = createToolRegistry([], { legacy }).list();
  assert.deepEqual(listed, [
    { name: "annotation_free", description: "as published", inputSchema: EMPTY_OBJECT_SCHEMA }
  ]);
  assert.equal("annotations" in (listed[0] ?? {}), false);
});
