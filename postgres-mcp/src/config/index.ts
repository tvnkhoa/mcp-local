 ...
 * `PG_EXPLAIN_COST_WARN` and TTLs where a silent change of parsing is a change of behaviour. The
 ...
 * A write gate.
 *
 * `strictFlag`, not `boolean`: `PG_WRITE_ENABLED` and `PG_MIGRATION_ENABLED` accept only an exact
...
/**
 * The raw `PG_WRITE_APPROVAL_SECRET`, before `resolveApprovalSecret` decides whether to generate
 * one. Empty string when unset, which is what triggers per-process generation.
 */
export function approvalSecretFromEnv(): string {
  return envReader.string("PG_WRITE_APPROVAL_SECRET", "");
}
...
export function dotnetProjectsFromEnv(): { project: string; startupProject: string } {
  return {
    project: stringFromEnv("CH_DOTNET_PROJECT"),
    startupProject: stringFromEnv("CH_DOTNET_STARTUP_PROJECT")
  };
}
