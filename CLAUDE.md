...
- `bitbucket-mcp/` — Bitbucket Cloud MCP server. Reads repositories/pull requests/**pipelines** and **creates pull requests** (`list_repositories`, `get_repository`, `list_branches`, `list_pull_requests`, `get_pull_request`, `get_pull_request_diff`, `list_pipelines`, `get_pipeline`, `list_pipeline_steps`, `get_pipeline_step_log`, `create_pull_request`). The four pipeline tools are **read-only** — there is no trigger/stop; they walk one CI failure to its step log, and `get_pipeline_step_log` returns the bounded **tail** (default 256 KiB, max 1 MiB) via an HTTP `Range` request. Uses scopes `read:repository` / `read:pullrequest` / `read:pipeline` / `write:pullrequest`. Auth via env (`BITBUCKET_ACCESS_TOKEN` Bearer, or `BITBUCKET_EMAIL`+`BITBUCKET_API_TOKEN` Basic). **PR creation is OFF unless `BITBUCKET_WRITE_ENABLED=true`**; `create_pull_request` supports `dryRun`. Registered as `bitbucket-mcp` in `~/.claude.json`.
